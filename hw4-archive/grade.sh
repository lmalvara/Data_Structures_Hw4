...
grade=$baseGrade
for casenum in `seq 1 1 $casecount`; do
    timeout -k $timeout $timeout ./$filename "preorder=pre$casenum.txt;inorder=in$casenum.txt" 1>$casenum.stdout 2>$casenum.stderr
    if [ $? -ne 0 ]; then
        echo -e "        ${red}Program killed due to timeout ($timeout).$nocolor"
        echo "Test case $casenum timed out ($timeout)." >> TIMEOUT
    fi

    diff -iEBwu $casenum.ans $casenum.stdout > $casenum.diff
    if [ $? -ne 0 ]; then
